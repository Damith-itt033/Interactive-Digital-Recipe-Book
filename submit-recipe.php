...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($category_id) || empty($ingredients) || empty($instructions)) {
        ...
        if ($stmt->execute([$user_id, $category_id, $title, $instructions, $ingredients, $prep_time, $cook_time, $servings, $difficulty, $image_url, $calories])) {
            $msg = "Success! Your recipe has been shared with the community.";
            $msgType = "success";
        } else {
            $msg = "An error occurred while saving your recipe.";
            $msgType = "danger";
        }
    }
}
...
<head>
  ...
  <title>Savory Share - Submit Recipe</title>
